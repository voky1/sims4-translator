...
class Exp(namedtuple('Expansion', 'names folder')):
    ...
    def status(self) -> str:
        if self.exists:
            if not self.exists_source:
                return interface.text('OptionsDialog', '{} not exist').format(Expansion.strings_source)
            elif not self.exists_dest:
                return interface.text('OptionsDialog', '{} not exist').format(expansion.strings_dest)
            else:
    ...
    def file_source(self) -> str:
        return str(
            os.path.join(config.value('dictionaries', 'gamepath'),
                         self.folder, expansion.strings_source + '.package'))

    @property
    def file_dest(self) -> str:
        return str(os.path.join(config.value('dictionaries', 'gamepath'),
                                self.folder, expansion.strings_dest + '.package'))
...
class Expansion(metaclass=Singleton):
    ...
    def exists(self) -> List[Exp]:
        return [exp for exp in self.items if isinstance(exp, Exp) and exp.exists_strings]
