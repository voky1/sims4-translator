...
from windows.ui.translatedialog import Ui_TranslateDialog
...
class BatchTranslateWorker(QRunnable):
    ...
    def __init__(self, chunk: Union[MainRecord, List[MainRecord]], engine: str, undo):
        super().__init__()
    ...
    def run(self):
        chunk = self.chunk
        is_fast = not isinstance(self.chunk, MainRecord)
        ...
        if is_fast:
            ...
            combined_text = text_to_edit(self.chunk.source)
...
class TranslateDialog(QDialog, Ui_TranslateDialog):
    ...
    def translate(self):
        ...
        if self.rb_selection.isChecked():
            ...
            items = self.main_window.main_model.model.items
        ...
        for chunk in chunk_items:
            worker = BatchTranslateWorker(chunk, self.cb_api.currentText(), self.main_window.undo)
    ...
    def __error_translate_chunk(self, text: str):
        self.__log.append('<span style="color: red;">' + text + '</span>')
        self.print_log()
        self.stop_translate()
